                ...
                <tbody>
                    @foreach ($tickets as $key => $ticket)
                        @if ($ticket->user != null)
                            <tr>
                                <td>#{{ $ticket->code }}</td>
                                <td>{{ convert_datetime_to_local_timezone($ticket->created_at, $timezone) }} @if ($ticket->viewed == 0)
                                ...
                                <td>
                                    @if (count($ticket->ticketreplies) > 0)
                                        {{ convert_datetime_to_local_timezone($ticket->ticketreplies->last()->created_at, $timezone) }}
                                    @else
                                        {{ convert_datetime_to_local_timezone($ticket->created_at, $timezone) }}
                                    @endif
                                </td>
                                <td class="text-right">
