...
@section('title', translate('Edit Voucher Information'))
@section('content')
    ...
    <div class="col-lg-11 mx-auto">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('merchants-vouchers.update', $merchant_voucher->id) }}" method="POST"
                            ...
                            <select class="form-control aiz-selectpicker" name="merchant_id" id="merchant_id"
                                ...
                                @foreach ($merchants as $merchant)
                                    <option value="{{ $merchant->id }}"
                                        {{ $merchant->id == $merchant_voucher->merchant_id ? 'selected' : '' }}>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9 input-container">
                            <input type="text" placeholder="{{ translate('unique voucher code') }}" id="voucher_code"
                                name="voucher_code" class="form-control input-field"
                                value="{{ $merchant_voucher->voucher_code }}" required>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="number" placeholder="{{ translate('total no of usage like 10,5,4') }}"
                                id="total_limit" name="total_limit" class="form-control"
                                value="{{ $merchant_voucher->total_limit }}" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="number" step="any" placeholder="{{ translate('discount to be given') }}"
                                name="amount" value="{{ $merchant_voucher->amount }}" class="form-control" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-3 col-from-label">{{ translate('Description') }}</label>
                        <div class="col-sm-9">
                            <textarea class="aiz-text-editor" name="description">{{ $merchant_voucher->description }}</textarea>
                        </div>
                    </div>
                    <div class="form-group row">
