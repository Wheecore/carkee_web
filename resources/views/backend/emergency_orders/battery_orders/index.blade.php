...
@section('title', translate('Battery Orders'))
@section('content')
    ...
    <div class="card">
        <form class="" action="" id="sort_orders" method="GET">
            <div class="card-header row gutters-5">
                <div class="col">
                    <h5 class="mb-md-0 h6">{{ translate('Battery Orders') }}</h5>
                </div>
                        ...
                        <tbody>
                            @foreach ($orders as $key => $order)
                                <tr>
                                    ...
                                    <td>{{ $order->battery_type == 'N' ? 'New Battery' : 'Jumpstart' }}</td>
                                    ...
                                    <td style="white-space: nowrap;">
                                        @if ($order->delivery_status != 'completed' && $order->delivery_status != 'cancelled')
                                            <button type="button" class="btn btn-primary btn-xs"
                                                ...
                                                data-toggle="modal" data-target="#changeStatusModal">{{ translate('Change Status') }}</button>
                                        @endif
                                        <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                            href="{{ route('battery_orders.show', encrypt($order->id)) }}"
                                            title="{{ translate('View') }}">
                                            <i class="las la-eye"></i>
                                        </a>
                                        <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                            href="{{ route('invoice.emergency-order', ['order_id' => $order->id, 'type' => 'battery']) }}"
