                ...
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('business_settings.update') }}" method="POST">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-6">
                                <select class="form-control aiz-selectpicker" name="system_default_currency"
                                    data-live-search="true">
                                    @foreach ($active_currencies as $key => $currency)
                                        <option value="{{ $currency->id }}" <?php if (get_setting('system_default_currency') == $currency->id) {
                                            echo 'selected';
                                        } ?>>
                                            {{ $currency->name }}
                                        </option>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-6">
                                <select class="form-control aiz-selectpicker" name="symbol_format">
                                    <option value="1" @if (get_setting('symbol_format') == 1) selected @endif>
                                        [Symbol][Amount]</option>
                                    <option value="2" @if (get_setting('symbol_format') == 2) selected @endif>
                                        [Amount][Symbol]</option>
                                    <option value="3" @if (get_setting('symbol_format') == 3) selected @endif>[Symbol]
                                        [Amount]</option>
                                    <option value="4" @if (get_setting('symbol_format') == 4) selected @endif>[Amount]
                                        [Symbol]</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-lg-6">
                                <select class="form-control aiz-selectpicker" name="decimal_separator">
                                    <option value="1" @if (get_setting('decimal_separator') == 1) selected @endif>1,23,456.70
                                    </option>
                                    <option value="2" @if (get_setting('decimal_separator') == 2) selected @endif>1.23.456,70
                                    </option>
                                </select>
                            </div>
                        </div>
                            ...
                            <td>
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input onchange="update_currency_status(this)" value="{{ $currency->id }}"
                                        type="checkbox" <?php if ($currency->status == 1) {
                                            echo 'checked';
                                        } ?>>
