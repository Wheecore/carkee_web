...
@section('title', translate('Product Reviews'))
@section('content')
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="javascript::void(0);" target="_blank"
                                    class="text-reset text-truncate-2">{{ $review->product }}</a>
                            </td>
                            <td>{{ $review->added_by }}</td>
                            <td>{{ $review->name }} ({{ $review->email }})</td>
                            <td>{{ $review->rating }}</td>
                            <td>{{ $review->comment }}</td>
                            <td>
                                <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{ route('reviews.destroy', $review->id) }}" title="{{ translate('Delete') }}">
