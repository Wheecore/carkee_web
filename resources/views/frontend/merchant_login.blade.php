...
@section('content')
    ...
    <div class="container-fluid full-body" style="padding: 0px;">
        ...
        <div class="left">
            <div class="header">
                <h2 class="animation a1">{{ translate('Login to your Merchant account.') }}</h2>
            </div>
            <form class="form-default" role="form" action="{{ route('login') }}" method="POST">
                @csrf
                <div class="form">
                    @if (DB::table('addons')->where('unique_identifier', 'otp_system')->first() != null &&
                        DB::table('addons')->where('unique_identifier', 'otp_system')->first()->activated)
                        <input type="text"
                            class="form-field animation a3 {{ $errors->has('email') ? ' is-invalid' : '' }}"
