<html>
  ...
  <body>
    <form method="post" name="ePayment" id="ePayment" action="https://payment.ipay88.com.my/ePayment/entry.asp">
      ...
      <input type="hidden" name="RefNo" value="{{ $id }}">
      <input type="hidden" name="Amount" value="{{ $amount }}">
      <input type="hidden" name="Currency" value="MYR">
      <input type="hidden" name="ProdDesc" value="Ref #{{ $id }}">
      <input type="hidden" name="UserName" value="{{ $user->name }}">
      <input type="hidden" name="UserEmail" value="{{ $user->email }}">
      <input type="hidden" name="UserContact" value="{{ $user->phone }}">
      <input type="hidden" name="Remark" value="{{ $data_arr }}">
      <input type="hidden" name="Lang" value="UTF-8">
      <input type="hidden" name="SignatureType" value="SHA256">
      <input type="hidden" name="Signature" value="{{ $hash }}">
      <input type="hidden" name="ResponseURL" value="{{ url('api/v2/api/ipay88/response') }}">
      <input type="hidden" name="BackendURL" value="{{ url('api/v2/api/ipay88/backend') }}">
