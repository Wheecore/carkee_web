...
class CartController extends Controller
{
            ...
            if (!is_null($cart->product_id)) {
                ...
                if ($product) {
                    ...
                    $data['items'][] = [
                        ...
                        'qty_1_price' => ($product->category_name == "Tyre") ? homeDiscountedBasePrice($product, $product->quantity_1_price) : homeDiscountedBasePrice($product, $product->unit_price),
                        // 'greater_1_price' => homeDiscountedBasePrice($product, $product->greater_1_price),
                        'greater_1_price' => ($product->category_name == "Tyre") ? homeDiscountedBasePrice($product, $product->greater_1_price) : homeDiscountedBasePrice($product, $product->unit_price),
                        // 'greater_3_price' => homeDiscountedBasePrice($product, $product->greater_3_price),
                        'greater_3_price' => ($product->category_name == "Tyre") ? homeDiscountedBasePrice($product, $product->greater_3_price) : homeDiscountedBasePrice($product, $product->unit_price),
    ...
    public function search(Request $request, $category_id = null, $featured_id = null, $sub_featured_id = null, $vehicle_category_id = null, $user_id)
    {
        ...
        $featured_categories_arr = $featured_categories->map(function ($featured_category) {
            $featured_sub_categories = FeaturedSubCategory::where('featured_category_id', $featured_category->id)->select('id', 'name')->orderBy('name','asc')->get();
